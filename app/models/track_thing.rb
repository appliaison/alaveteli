...
class TrackThing < ActiveRecord::Base
    ...
    def track_query_description
        parsed_query = self.track_query.gsub('(variety:sent OR variety:followup_sent OR variety:response OR variety:comment)', '')
        if parsed_query != self.track_query
            parsed_query.strip!
            if parsed_query.empty?
                return _("all requests or comments")
            else
                return _("all requests or comments matching text '{{query}}'", :query => parsed_query)
            end
        end
        parsed_query = self.track_query.gsub('(latest_status:successful OR latest_status:partially_successful)', '')
        if parsed_query != self.track_query
            parsed_query.strip!
            if parsed_query.empty?
                return _("requests which are successful")
            else
                return _("requests which are successful matching text '{{query}}'", :query => parsed_query)
            end
        end
        return _("anything matching text '{{query}}'", :query => self.track_query)
    end
