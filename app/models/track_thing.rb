    ...
    def track_query_description
        ...
        for filter in filters
            ...
            if filter =~ /(rejected|not_held)/
                statuses << _("unsuccessful requests")
                match = true
            end
            if filter =~ /(:successful|:partially_successful)/
                statuses << _("successful requests")
                match = true
            end
            if filter =~ /waiting/
                statuses << _("requests awaiting a response")
                match = true
            end
    ...
    def params
        if @params.nil?
            if self.track_type == 'request_updates'
                @params = {
                    # Website
                    :subscribe_email => _("You will now be emailed updates about '{{link_to_request}}', a request.",
                                         :link_to_request => request_link(self.info_request)),
                    :already_subscribed_description => _("You are already following updates about '{{link_to_request}}', a request.",
                                                        :link_to_request => request_link(self.info_request)),
                    :unsubscribed_description => _("You are no longer following '{{link_to_request}}', a request.",
                                                :link_to_request => request_link(self.info_request)),
                ...
                @params = {
                    # Website
                    :subscribe_email => _("You will now be emailed updates about <a href=\"/list\">new requests</a>"),
                    :already_subscribed_description => _("You are already following <a href=\"/list\">new requests</a>"),
                    :unsubscribed_description => _("You are no longer following <a href=\"/list\">new requests</a>"),
                ...
                @params = {
                    # Website
                    :subscribe_email => _("You will now be emailed updates about <a href=\"/list/successful\">successful requests</a>"),
                    :already_subscribed_description => _("You are already following <a href=\"/list/successful\">successful requests</a>"),
                    :unsubscribed_description => _("You are no longer following <a href=\"/list/successful\">successful requests</a>"),
                ...
                @params = {
                    # Website
                    :subscribe_email => _("You will now be emailed updates about '{{link_to_authority}}', a public authority",
                                        :link_to_authority => public_body_link(self.public_body)),
                    :already_subscribed_description => _("You are already following '{{link_to_authority}}', a public authority",
                                                        :link_to_authority => public_body_link(self.public_body)),
                    :unsubscribed_description => _("You are no longer following '{{link_to_authority}}', a public authority",
                                            :link_to_authority => public_body_link(self.public_body)),
                    :list_description => _("'{{link_to_authority}}', a public authority",
                                            :link_to_authority => public_body_link(self.public_body)),
                    ...
                    :verb_on_page_already => _("You are already following requests to {{public_body_name}}",
                                                :public_body_name => self.public_body.name),
                    # Email
                    :title_in_email => _("{{law_used}} requests to '{{public_body_name}}'",
                                        :law_used => self.public_body.law_only_short,
                                        :public_body_name => self.public_body.name),
                    :title_in_rss => _("{{law_used}} requests to '{{public_body_name}}'",
                                        :law_used => self.public_body.law_only_short,
                                        :public_body_name => self.public_body.name),
                    # Authentication
                ...
                @params = {
                    # Website
                    :subscribe_email => _("You will now be emailed updates about '{{link_to_user}}', a person",
                                        :link_to_user => user_link(tracked_user)),
                    :already_subscribed_description => _("You are already following '{{link_to_user}}', a person",
                                                         :link_to_user => user_link(tracked_user)),
                    :unsubscribed_description => _("You are no longer following '{{link_to_user}}', a person",
                                            :link_to_user => user_link(tracked_user)),
                    :list_description => _("'{{link_to_user}}', a person",
                                            :link_to_user => user_link(tracked_user)),
                ...
                search_url = "/search/" + CGI.escapeHTML(self.track_query) + "/newest/advanced"
                @params = {
                    # Website
                    :subscribe_email => _('You will now be emailed updates about this search.'),
                    :already_subscribed_description => _('You are already following <a href="{{search_url}}">this search</a>.',
                                                        :search_url => search_url),
                    :unsubscribed_description => _('You are no longer following this search.'),
                    :list_description => self.track_query_description,
